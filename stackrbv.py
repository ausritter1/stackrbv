...
# Replace with the path to Red Beard Ventures logo
logo_path = "logo.png"
...
acceptable_locations = st.sidebar.multiselect(
    ...
    ["USA", "Canada", "UK", "Germany", "China", "Other"],
    default=["USA", "Canada"]
)
...
def remove_startup(df, startup_name):
    print(f"Attempting to remove: {startup_name}")
    removed_df = df[df['Startup Name'] != startup_name]
    print(f"DataFrame after removal:\n{removed_df}")
    return removed_df
...
with st.sidebar.form(key='add_startup_form'):
    ...
    industry = st.selectbox("Industry", ["Fintech", "Healthcare", "AI", "Other"])
    ...
    location = st.selectbox("Location", ["USA", "Canada", "UK", "Germany", "China", "Other"])
